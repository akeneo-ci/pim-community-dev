...
class ProductController
{
    ...
    public function partialUpdateAction(Request $request, $code): Response
    {
        ...
        if ($this->needUpdateFromProductToVariant($product, $data, $isCreation)) {
            try {
                $product = $this->addParent->to($product, $data['parent']);
    ...
    {
        return !$isCreation && !$product instanceof VariantProductInterface && isset($data['parent']) &&
            null !== $data['parent'] && '' !== $data['parent'];
    }
}
